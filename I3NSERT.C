...
colour_term construct_insertion_op(process m_process, unsigned int lno1, unsigned int lno2) {
    ...
    if (m_process.leg(lno1).second==21) {
        antisymmetric.set_indices(lno1,1001,lno1+2000);
        prefactor*=complex<double>(0.,1.);
    }
    else if ((m_process.leg(lno1).second>=1 and m_process.leg(lno1).second<=6 and m_process.is_in_leg(lno1)) or (m_process.leg(lno1).second<=-1 and m_process.leg(lno1).second>=-6 and !m_process.is_in_leg(lno1))) { 
        fundamental.set_indices(1001,lno1+2000,lno1);
        prefactor*=-1;
    }
    else if ((m_process.leg(lno1).second>=1 and m_process.leg(lno1).second<=6 and !m_process.is_in_leg(lno1)) or (m_process.leg(lno1).second<=-1 and m_process.leg(lno1).second>=-6 and m_process.is_in_leg(lno1))) fundamental.set_indices(1001,lno1,lno1+2000);
    else {
    ...
    if (m_process.leg(lno2).second==21) { 
        antisymmetric.set_indices(lno2,1001,lno2+2000);
        prefactor*=complex<double>(0.,1.);
    }
    else if ((m_process.leg(lno2).second>=1 and m_process.leg(lno2).second<=6 and m_process.is_in_leg(lno2)) or (m_process.leg(lno2).second<=-1 and m_process.leg(lno2).second>=-6 and !m_process.is_in_leg(lno2))) {
        fundamental.set_indices(1001,lno2+2000,lno2);
        prefactor*=-1;
    }
    else if ((m_process.leg(lno2).second>=1 and m_process.leg(lno2).second<=6 and !m_process.is_in_leg(lno2)) or (m_process.leg(lno2).second<=-1 and m_process.leg(lno2).second>=-6 and m_process.is_in_leg(lno2))) fundamental.set_indices(1001,lno2,lno2+2000);
    else { 
    ...
    for (unsigned int lno(1);lno<=m_process.no_of_legs();lno++) {
        if (lno!=lno1 and lno!=lno2) {
            if (m_process.leg(lno).second==21) gluonic=true;
            else gluonic=false;
